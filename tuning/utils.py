...
def read_audio(path: str,
    ...
    wav, sr = torchaudio.load(
        path,
        format='opus' if 'opus' in path else None,
        backend='soundfile' if 'opus' in path else None)
...
class SileroVadDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        timeend = time.time()
        print('Time to load audio per step', timeend - timestart)
        if self.mode == 'train':
...
def train(config,
    ...
    start_idx = step_of_epoch
    print("step_of_epoch: ", step_of_epoch)
    with torch.enable_grad():
        for _, (x, targets, masks) in tqdm(enumerate(loader), total=len(loader), initial=start_idx):
            if _ < start_idx:
                # Skip items until we reach the desired index
                print("_ is ", _)
                continue
            ...
            if step % save_interval == 0:
                ...
                torch.save({
                    'model_state_dict': jit_model.state_dict(),
...
def get_checkpoint():
    parser = argparse.ArgumentParser(description="Checking mode...")
    parser.add_argument('-m', type=str, help="Mode to run: 'load_checkpoint'", nargs='?', default='')

    args = parser.parse_args()

    if args.m == 'load_checkpoint':
        return True
    elif args.m == '':
        return False
    elif args.m != 'load_checkpoint':
        print('dont have this mode')
        raise SystemExit
